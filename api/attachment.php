...
$member->execute([(int)$a['chat_id'], (int)$user['id']]);
if (!$member->fetch()) fail('Not a member', 403);

$authorized = (int)$a['sender_id'] === (int)$user['id'] || $a['download_policy'] === 'ALLOW';
if (!$authorized && $a['download_policy'] === 'APPROVAL_REQUIRED') {
 ...
 $authorized = (bool)$q->fetch();
}
if (!$authorized) fail($a['download_policy'] === 'VIEW_ONLY' ? 'Download is disabled for this attachment' : 'Download requires sender approval', 403);
...
header('Content-Disposition: attachment; filename="' . addcslashes(basename($a['original_filename']), '"\\') . '"');
